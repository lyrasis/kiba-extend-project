...
module KeProject
  ...
  module RegistryData
    ...
    def register
      ...
      #   on the registry.
      #
      # :transform transforms the file hashes below into
      #   Kiba::Extend::Registry::FileRegistryEntry objects.
      #
      # :freeze ensures data in registry is immutable for the rest of the
      #   application's run.
      #
    ...
    def register_type_prep_jobs
      ...
      KeProject.registry.namespace("type") do
        KeProject.type_tables.each do |typetable, field|
          ...
          #   undefined error if we just did:
          #
          # ```
          ...
          #   `register_type_prep_jobs` definition, that we can then use to
          #   interact with the original scope/context.
          register typetable,
            bind.receiver.send(:typetable_reg_entry_hash, typetable, field)
        end
      end
    end
    ...
    #
    # The namespace hierarchy is completely arbitrary. Set it up (or not) in a
    #   way that makes sense to you.
    #
    # Caveat/thing to consider: Thor doesn't support autocomplete, and you will
    #   end up typing these job keys a lot, so I tend to avoid nested namespaces
    #   or long namespace names/job keys.
    #
    # If you use namespaces, the unique job key is built by joining all levels
    #   of namespace, plus the registered task name, using `__` (2 underscores)
    #   as the join delimiter.
    #
    ...
    def register_files
      ...
      #   all.
      #
      # It also demonstrates all the required pieces of a job definition, busted
      #   out into granular pieces for explanation. It also demonstrates that
      #   how you name things is completely arbitrary.
      #
      ...
      # The namespace below defines registry entries set up using the structure
      ...
      KeProject.registry.namespace("locations") do
        ...
        # All the other registry entries for jobs output CSV, which is our
        #   project's default destination type.
        #
