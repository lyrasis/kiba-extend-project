...
Everything in the https://github.com/lyrasis/kiba-extend-project/tree/main/lib/ke_project/jobs[/lib/ke_project/jobs] is an example of how job transformation logic is typically set up.
The https://github.com/lyrasis/kiba-extend-project/blob/main/lib/ke_project/everything_exploded.rb[everything_exploded.rb] module is a heavily commented, deconstructed job demonstrating how each piece of a job works.
It is not recommended that you ever set anything up this way in real life, but it gives some examples of how to call external code from within a `Kiba.job_segment` definition, which can be useful.

toc::[]

== Prerequisites

You must have a modern Ruby installed. This should work with 3.4.1 and up.
...
Pro:: less tedious deletion of comments and code you don't need
